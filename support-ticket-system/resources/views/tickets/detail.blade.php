...
<div class="p-12 max-w-4xl">
    ...
    <div class="mb-8">
        <h1 class="text-4xl font-bold text-gray-900">Ticket Details</h1>
    </div>
    ...
    <form action="{{ route('tickets.update', $ticket->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <div class="flex gap-3">
                <select 
                    name="user_id" 
                    id="assignee-select"
                    selected="{{ old('user_id', $ticket->user->name) }}"
                ...
                @foreach($users as $user)
                    <option 
                        value="{{ $user->id }}"
                        {{ (old('user_id', $ticket->user_id) == $user->id)}}
                    >
                        {{ $user->name }}
                    </option>
                @endforeach
        ...
        <div>
            ...
            <button 
                type="button" 
                onclick="window.location='{{ route('tickets') }}'"
                class="ml-4 bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold px-8 py-3 rounded-lg transition-colors shadow-sm"
            >
                Cancel
            </button>
        </div>
